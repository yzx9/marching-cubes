...
namespace vec
{
    ...
    public:
        ...
        Vec3<T> operator+(const Vec3<T> &v) const { return Vec3<T>{data[0] + v.data[0], data[1] + v.data[1], data[2] + v.data[2]}; };
        Vec3<T> operator-(const Vec3<T> &v) const { return Vec3<T>{data[0] - v.data[0], data[1] - v.data[1], data[2] - v.data[2]}; };
    ...
    {
        return Vec3<T>{v1[0] + (v2[0] - v1[0]) * interpolation,
    ...
    template <typename T>
    inline T distance2(const Vec3<T> &v1, const Vec3<T> &v2)
    {
    ...
    template <typename T>
    inline T distance(const Vec3<T> &v1, const Vec3<T> &v2) { return std::sqrt(distance2(v1, v2)); }
    ...
    public:
        ...
        T &operator[](int i) { return data[i]; };
        const T &operator[](int i) const { return data[i]; };
        Vec4<T> operator+(const Vec4<T> &v) const { return Vec4<T>{data[0] + v.data[0], data[1] + v.data[1], data[2] + v.data[2], data[3] + v.data[3]}; };
        Vec4<T> operator-(const Vec4<T> &v) const { return Vec4<T>{data[0] - v.data[0], data[1] - v.data[1], data[2] - v.data[2], data[3] - v.data[3]}; };
        T operator*(const Vec4<T> &v) const { return data[0] * v.data[0] + data[1] * v.data[1] + data[2] * v.data[2] + data[3] * v.data[3]; };
        Vec4<T> operator*(const matrix::SymmetryMatrix4<T> &m) const
        {
            return Vec4<T>{data[0] * m(0, 0) + data[1] * m(0, 1) + data[2] * m(0, 2) + data[3] * m(0, 3),
    ...
    template <typename T>
    inline T distance2(const Vec4<T> &v1, const Vec4<T> &v2)
    {
        return (v1[0] - v2[0]) * (v1[0] - v2[0]) +
               (v1[1] - v2[1]) * (v1[1] - v2[1]) +
               (v1[2] - v2[2]) * (v1[2] - v2[2]) +
    ...
    template <typename T>
    inline T distance(const Vec4<T> &v1, const Vec4<T> &v2) { return std::sqrt(distance2(v1, v2)); }
}
